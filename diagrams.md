...
flowchart TD
    ...
    SEARCH[search.py<br/>searchIntent, question<br/>bookDescSearch<br/>Cosine Similarity]
  
    CONTEXT[context.py<br/>sessionContext<br/>resolveEllipsis<br/>updateContext]
  
    HANDLERS[handlers.py<br/>small, discover, identity<br/>reccomend, check, opening<br/>address, facilities, locations<br/>stockCheck, thank]
  
    ORDERS[orders.py<br/>order, detectCorrection<br/>handleInputWithIntents<br/>getBookSelection, getQuantitySelection<br/>getPickupOrDelivery, getLocationSelection<br/>getDeliveryAddress, getPickupDate, getPickupTime<br/>storeOrder, collectFeedback]
  
    NLG[nlg.py<br/>getReferringExpression<br/>aggregateOrderDetails<br/>generateContextualError<br/>addDiscourseMarker]
  
    DATA[dataAccess.py<br/>fuzzySearchTitle<br/>stockCheck, readName, writeName<br/>getStockJSON, getLocationsJSON<br/>getOrdersJSON]
  
    UTILS[utils.py<br/>confirmation, wordToInt<br/>levenshteinDistance]
  
    DATASETS[(intents.csv<br/>qa.csv<br/>stock.json<br/>locations.json<br/>orders.json<br/>session.json<br/>feedback.json)]
    ...
    HANDLERS --> NLG & DATA & UTILS & CONTEXT
    ORDERS --> NLG & DATA & UTILS & CONTEXT
    DATA --> DATASETS
    HANDLERS --> DATASETS
    ORDERS --> DATASETS
    NLG --> UI
...
flowchart TD
    ...
    ROUTE --> HANDLER["Route to Intent Handler:<br/><br/>• small - Small talk<br/>• discover - Feature discovery<br/>• identity - Name management<br/>• question - Q&A search<br/>• order - Transaction flow<br/>• reccomend - Genre recommendation<br/>• check - Order checking<br/>• opening - Store hours<br/>• address - Store address<br/>• facilities - Store facilities<br/>• locations - List locations<br/>• stockCheck - Stock availability<br/>• thank - Gratitude response"]
  
    HANDLER --> END([Response to User])
```
...
stateDiagram-v2
    [*] --> GetBook
    GetBook --> GetQuantity
    GetQuantity --> ChooseMethod
    
    state ChooseMethod <<choice>>
    ChooseMethod --> Pickup: pickup
    ChooseMethod --> Delivery: delivery
    ...
    Confirm --> StoreOrder
    StoreOrder --> CollectFeedback
    CollectFeedback --> [*]
    
    note right of GetBook
        Fuzzy matching with tolerance = len(title) // 3
        Auto-confirm if distance ≤ 2
        Ask confirmation if 2 < distance ≤ tolerance
        Max 3 attempts before cancellation
    end note
    
    note right of ChooseMethod
        detectCorrection() allows users to correct
        book or quantity at any subsequent step
    end note
    
    note right of GetDate
        Supports date ranges, natural language
        (25th December), relative (tomorrow),
        and numeric formats (DD/MM/YYYY)
    end note
    
    note right of GetTime
        Supports time ranges, 12-hour (9am),
        24-hour (14:00), and general times
        (noon, morning)
    end note
```
...
flowchart LR
    ...
    subgraph "Handler Layer"
        ...
        SMALL_H[Small Talk]
        IDENTITY_H[Identity Handler]
        ORDER_H[Order Handler]
        RECCOMEND_H[Reccomend Handler]
        QA_H[Q&A Handler]
    ...
    subgraph "Data Access Layer"
        ...
        SESSION_DB[(session.json<br/>User identity & genre)]
        INTENT_DB[(intents.csv<br/>Training examples)]
        QA_DB[(qa.csv<br/>Q&As)]
        FEEDBACK_DB[(feedback.json<br/>User feedback)]
    end
    ...
    SMALL_H <--> SESSION_DB
    IDENTITY_H <--> SESSION_DB
    ORDER_H <--> STOCK_DB
    ORDER_H <--> LOC_DB
    ORDER_H --> ORDER_DB
    ORDER_H --> FEEDBACK_DB
    RECCOMEND_H <--> STOCK_DB
    RECCOMEND_H --> SESSION_DB
    INFO_H <--> LOC_DB
    INFO_H <--> STOCK_DB
    QA_H <--> QA_DB
